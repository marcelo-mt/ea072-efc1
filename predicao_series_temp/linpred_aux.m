...
function aux_plot(i, c, range, expected, measured, rEQM)

	figure(i);
	p = plotyy(range, expected, range, measured)
	title(sprintf('Validação do preditor linear para c = %f', c));
	xlabel(sprintf('Erro rEQM = %f', rEQM));
	ylabel(p(1), 'Dados de Validação');
	ylabel(p(2), 'Valores Calculados');
	grid;
...
function [b_coefs, expected, measured, rEQM] = linear_prediction(X_train, S_train, X_valid, S_valid, c)
	...
	measured = validate(X_valid, b_coefs);
	expected = S_valid;
...
function X_pred = validate(X, b_coefs)
	...
	for i = 1 : size(X,1),
		...
		x(n) = 1;
		past_values = x';
		X_pred = vertcat(X_pred, predict(past_values, b_coefs));
