...
for i = 1 : k,
	...
	for j = 1 : k,
    	...
    	if i == j,
    		...
    		X_test = vertcat(X_test, X);
    		S_test = vertcat(S_test, S);
    	end
	end
    ...
    % Appending a Column of 1's to the validation data
	n_lines = size(X_validation, 1);
	X_validation = horzcat(X_validation, ones(n_lines, 1));

    % Saving test
	X = X_validation;
    S = S_validation;
    save(strcat(input_root_file, sprintf('_validation_%d', i)), 'X','S');
